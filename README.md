...
The web preview output is generated under `web/build/dist/js/productionExecutable/`. It currently supports in-memory NIP-07 public-key sign-in, in-memory NIP-46 `bunker://` remote-signer public-key sign-in, read-only note loading, basic signer-backed note create/edit/delete, text-only profile metadata display, session-only note relay selection, and session-only import of the active user's published kind `10002` write relays when the active signer exposes the needed NIP-44 and event-signing capabilities. Browser session persistence, durable remote-signer sessions, direct `nsec` input, durable note caches, persistent pending writes, native-style relay migration, durable relay preferences, and web relay-list publishing remain future work governed by [docs/web-client-architecture.md](docs/web-client-architecture.md). Web static hosting and CSP checks are documented in [docs/web-deployment-security.md](docs/web-deployment-security.md).
...
- `web`: standalone Kotlin/JS preview shell. It intentionally does not depend on the native app runtime. Current web auth is limited to in-memory NIP-07 and NIP-46 public-key sign-in, with in-memory note loading, signer-backed create/edit/delete, text-only profile metadata, session-only note relay selection, and session-only kind `10002` write-relay import when the active signer supports the required capabilities. Browser storage, durable relay preferences, native-style relay migration, web relay-list publishing, durable note caches, and persistent pending writes are not implemented yet.
