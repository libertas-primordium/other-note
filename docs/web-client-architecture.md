...
Current direct-key implementation status: the web runtime has a hidden, tested foundation that can decode a valid `nsec`, derive the account pubkey, encrypt/decrypt NIP-44 payloads to self, sign and validate kind `30078` note events, and clear the in-memory key bytes on logout/session replacement. It is not exposed through the signed-out UI yet. A future UI pass must use a password-style input, clear the input immediately after successful session creation, avoid putting the raw value in DOM attributes or URLs, and explain that refresh/logout forgets the session.
