...
On web, the direct `nsec` field may offer a default-off option that lets a compatible browser or password manager offer save/fill behavior through normal form/autocomplete semantics. That is user-controlled external storage, not Other Note app-controlled storage. Other Note must still not write direct `nsec` values, private keys, direct-key auth state, or direct-key sessions to localStorage, sessionStorage, IndexedDB, cookies, Cache Storage, logs, analytics, telemetry, URLs, hidden fields, or app preferences. Users should enable browser/password-manager save/fill only in a trusted browser profile or password manager, and refresh/logout must still clear Other Note's active session.
