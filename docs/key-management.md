...
Current web preview support is still intentionally narrow: NIP-07 and NIP-46 sign-in are in-memory only, and session-only direct `nsec` fallback sign-in is exposed only as a lower-emphasis option for the current page session. The direct `nsec` input is password-style, suppresses browser assistance where practical, clears the pasted draft on submit, and must not persist direct-key private material. Note loading plus basic create/edit/delete may fetch and publish encrypted kind `30078` note events through session-selected note relays when the active signer or direct-key session exposes NIP-44 encrypt/decrypt and event-signing capabilities. Web note relay selection is note-relay-only, session-only, can import the active user's latest published kind `10002` write relays for the current browser session, and must not display or edit NIP-46 signer transport relays. The web preview must not persist browser auth sessions, NIP-46 communication keys, direct-key private material, encrypted note events, decrypted note bodies, decrypted payload JSON, drafts, note relay preferences, pending writes, or direct `nsec` values.
