...
On Android and web, the direct `nsec` field may offer a default-off option that lets a compatible password manager offer save/fill behavior. That is user-controlled external password-manager storage, not Other Note app-controlled storage. Android uses an explicit Credential Manager password-save request after successful direct `nsec` validation; the request provides the public `npub` as the username/account identifier and the direct `nsec` as the password only when the user enabled that option. Other Note must still not write direct `nsec` values, private keys, direct-key auth state, or direct-key sessions to plaintext app preferences, localStorage, sessionStorage, IndexedDB, cookies, Cache Storage, logs, analytics, telemetry, URLs, hidden fields, relay settings, caches, or pending-write stores. Users should enable password-manager save/fill only in a trusted browser/device profile or password manager, and refresh/logout/process restart must still clear Other Note's active session unless the user separately continues with an explicitly saved identity.

Other Note may generate a fresh Nostr identity for a user-facing "Create new identity" flow. The generated `nsec` is the private key. It may be displayed only inside the deliberate generation flow so the user can save it outside Other Note, save it explicitly to Android Keystore-backed encrypted storage or the Linux desktop keyring where supported, or import it into a signer, and it must never be written to plaintext app preferences, DataStore, SQLite, files, JSON stores, durable relay caches, logs, analytics, crash reports, or relay events. The generated direct-key session remains session-only unless the user explicitly saves the key to supported OS-backed secure storage. When production crypto and a relay client are available, that session may use the in-memory private key for NIP-44 v2 encryption/decryption and NIP-01 signing, but durable storage must still contain only signed encrypted events and safe relay metadata. Losing the generated `nsec` means losing access to encrypted notes for that identity if no signer/keyring/password-manager copy exists.
...
Direct `nsec` paste may exist as a fallback. The direct `nsec` password-manager save option is off by default and is an explicit Credential Manager save request, not passive Autofill. When that option is enabled, Android derives the public `npub` only after successful validation and calls Credential Manager with `CreatePasswordRequest(id = npub, password = nsec)`. The user can accept or cancel the provider save sheet, and login continues either way. Provider behavior varies, so this is a supported Android password-manager request but still not a guarantee that Google Password Manager or any other provider is available on every device. Password-manager storage is external user-controlled storage, not Other Note app-controlled storage; the password manager may store the public `npub` together with the `nsec` if the user accepts its save prompt. Other Note must not save Android `nsec` values to plaintext app storage. If production crypto is available, direct session-only keys can encrypt/decrypt and sign notes in memory for relay publication and recovery; if production crypto is unavailable, the safety fallback remains local-only and must not emit plaintext.

The "Create new identity" flow can be used to generate an `nsec`, but the recommended Android path remains importing or saving that key in Amber or another NIP-55 signer and then using Android signer login in Other Note. Users who do not want to manage a raw `nsec` should prefer Android signer or NIP-46 remote signer/bunker.

Saved-device `nsec` support on Android uses a narrow Android Keystore-backed implementation. Other Note creates an AES-GCM key in Android Keystore and stores versioned encrypted `nsec` records in app-private preferences under a dedicated saved-direct-key store. The stored record may include schema version, account pubkey/derived npub metadata, ciphertext, nonce/IV, and timestamps. It must not contain plaintext `nsec`, raw private keys, note content, note events, relay events, relay settings, profile data, NIP-46 communication keys, bunker tokens, search/sort state, decrypted payload JSON, or diagnostics. Android backup and device-transfer extraction remain disabled for all app data; this is not a backup mechanism. If Android secure storage is unavailable or decryption fails, the UI must show safe recovery/forget copy and leave session-only direct `nsec` available.
...
Current web preview support is still intentionally narrow: NIP-07 sign-in is in-memory, NIP-46 sign-in is session-only by default with an explicit opt-in remembered remote-signer reconnect option, session-only direct `nsec` fallback sign-in is exposed only as a lower-emphasis option for the current page session, and fresh identity generation is a deliberate session-only recovery flow. The remembered web NIP-46 record may persist only the local communication private key, communication pubkey, remote signer pubkey, signer transport relay URLs, returned user pubkey, version, and timestamps under the generic key `on.web.nip46`; it must not persist the original bunker token secret, direct `nsec`, generated identity key, user private key, note content, note events, note relay settings, relay stats, profile data, search/sort state, or pending writes. The direct `nsec` input is password-style, suppresses browser assistance by default, offers only an explicit browser/password-manager form-hint opt-in, clears the pasted draft on submit, and must not persist direct-key private material. Browser/password-manager save/fill prompts are external user-controlled behavior and are not an Other Note saved session. The generated-identity flow shows the generated `nsec` only while the user saves it and acknowledges recovery risk; cancel, use-for-session, logout, refresh, and session replacement clear generated-key state. Note loading plus basic create/edit/delete may fetch and publish encrypted kind `30078` note events through session-selected note relays when the active signer or direct-key session exposes NIP-44 encrypt/decrypt and event-signing capabilities. Web note relay selection is note-relay-only, session-only, can import the active user's latest published kind `10002` write relays for the current browser session, and must not display or edit NIP-46 signer transport relays. Aside from the generic theme ID and explicit remembered NIP-46 record, the web preview must not persist browser auth sessions, generated or pasted direct-key private material, encrypted note events, decrypted note bodies, decrypted payload JSON, drafts, note relay preferences, profile data, search/sort state, relay stats, pending writes, or direct `nsec` values.
...
- Persisted web sessions beyond the explicit remembered NIP-46 communication-session exception, durable browser direct-key storage, or durable generated web identities.
